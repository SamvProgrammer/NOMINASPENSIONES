...
nSobres=''
*CLOSE TABLES all
aStruc= '(NOJUB   C( 9, 0), REPETIR  N( 1, 0),NOMBRE   C(40, 0), PERIODO  C( 30, 0), TPER C( 30, 0), FORMAP  C( 20, 0),'+;
...
FOR X1=1 TO 2
	         ...
	         do while (!EOF())
				...
				FOR xy=1 TO 2
		            for i= 1 to nmayor
					   ...
					   ELSE
						   replace periodo WITH 'ENERO - MAYO DE 2017'
						   replace tper WITH 'RETROACTIVO'
					   ENDIF
	...
	*   set relation to
	   select NomiNew
	   USE
		RutaNomina='Respaldos\nomi2017\NOMI07\RETRO\'
ENDFOR
*   set relation to
SET DATE BRITISH
SELECT (sobres)
